...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const add = (id: string, qty = 1, size?: string) => {
    setItems((prev) => {
      ...
      const allowed = availableFor(id, currentQty + qty, size as any) - currentQty;
      if (allowed <= 0) {
        toast.error("Not enough stock");
        return prev;
      }
      if (existing) {
        return prev.map((i) => (i === existing ? { ...i, qty: i.qty + allowed } : i));
      }
  ...
  const updateQty = (id: string, qty: number, size?: string) =>
    setItems((p) => p.map((i) => {
      if (i.productId === id && (size === undefined ? true : i.size === size)) {
