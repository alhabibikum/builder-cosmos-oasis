    ...
    return products
      .map((p) => {
        ...
        if (p.sizes && p.sizes.length) {
          ...
          return lows.length ? { id: p.id, title: p.title, sizes: lows, threshold: th } : null;
        }
        const qty = getStock(p.id);
        return qty <= th ? { id: p.id, title: p.title, sizes: [], threshold: th } : null;
      })
      .filter(Boolean) as { id: string; title: string; sizes: string[]; threshold: number }[];
  }, [products]);

  const totalUnits = useMemo(() => {
    return products.reduce((sum, p) => {
      if (p.sizes && p.sizes.length) return sum + p.sizes.reduce((s, sz) => s + getStock(p.id, sz as any), 0);
      return sum + getStock(p.id);
    }, 0);
  }, [products]);

  const history = getInventoryHistory();
  const chartData = useMemo(() => buildChartData(selected, history), [selected, history]);
    ...
    <div className="grid gap-6">
      ...
      <div className="rounded-xl border">
        ...
        <div className="divide-y">
          {lowStock.map((l) => (
            <div key={l.id} className="grid gap-2 p-3 md:grid-cols-3 md:items-center">
              <div>
              ...
              <div className="flex items-center gap-2 md:justify-end">
                ...
                <input
                  ...
                  onBlur={(e) => setThreshold(l.id, Number(e.target.value) || 0)}
                />
                <button className="rounded-md border px-2 py-1 text-xs" onClick={() => setSelected(l.id)}>
                  View Trends
                </button>
              </div>
            </div>
            ...
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
                  ...
                  <Tooltip />
                  <Line type="monotone" dataKey="qty" stroke="#0ea5e9" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          ) : (
            <div className="p-4 text-center text-sm text-muted-foreground">Select a product to view trends.</div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border">
        ...
        <div className="divide-y">
          {history.slice(0, 50).map((h, i) => (
            <div key={i} className="grid gap-1 p-3 text-sm md:grid-cols-5 md:items-center">
              <div className="md:col-span-2">
                <div className="font-medium line-clamp-1">{getProductById(h.id)?.title || h.id}</div>
                <div className="text-xs text-muted-foreground">{h.id}{h.size ? ` • ${h.size}` : ""}</div>
              </div>
              <div className={h.delta < 0 ? "text-red-600" : "text-green-600"}>{h.delta > 0 ? `+${h.delta}` : h.delta}</div>
              <div className="text-muted-foreground">Qty: {h.qtyAfter}</div>
              <div className="text-right text-xs text-muted-foreground">{new Date(h.at).toLocaleString()}</div>
            </div>
          ))}
          {history.length === 0 && (
    ...
    <div className="rounded-xl border">
      ...
      <div className="grid gap-3 p-3 md:grid-cols-5 md:items-end">
        <label className="grid gap-1 text-sm">
          ...
          <select className="h-10 rounded-md border px-2" value={pid} onChange={(e) => setPid(e.target.value)}>
        ...
        {p?.sizes?.length ? (
          <label className="grid gap-1 text-sm">
            ...
            <select className="h-10 rounded-md border px-2" value={size} onChange={(e) => setSize(e.target.value)}>
        ...
        <label className="grid gap-1 text-sm">
          ...
          <select className="h-10 rounded-md border px-2" value={mode} onChange={(e) => setMode(e.target.value as any)}>
            <option value="adjust">Adjust by ±</option>
            <option value="set">Set absolute</option>
          </select>
        </label>
        <label className="grid gap-1 text-sm">
          <span className="text-xs">{mode === "set" ? "Quantity" : "Delta"}</span>
          <input
            type="number"
        ...
        <label className="grid gap-1 text-sm md:col-span-2">
          ...
          <input className="h-10 rounded-md border px-2" value={reason} onChange={(e) => setReason(e.target.value)} />
        </label>
        <div className="md:col-span-5">
          <button className="rounded-md border px-3 py-2 text-sm" onClick={apply}>
            Apply
          </button>
        </div>
      </div>
