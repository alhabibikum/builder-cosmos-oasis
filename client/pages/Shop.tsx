...
export default function Shop() {
  ...
  const q = useMemo(() => new URLSearchParams(search).get("search")?.toLowerCase() || "", [search]);
  const filtered = useMemo(
    () => (q ? products.filter((p) => p.title.toLowerCase().includes(q) || p.tags?.some((t) => t.includes(q))) : products),
    [q],
  );
    ...
    <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            {Array.from(new Set(products.map((p) => p.category))).map((c) => (
              <li key={c}><Link to={`/shop?search=${encodeURIComponent(c)}`} className="hover:text-foreground">{c}</Link></li>
            ))}
          </ul>
        </div>
        <div>
      ...
      <section className="md:col-span-3">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="font-['Playfair Display'] text-2xl font-extrabold tracking-tight md:text-3xl">{q ? `Search: ${q}` : 'Shop'}</h1>
        </div>
