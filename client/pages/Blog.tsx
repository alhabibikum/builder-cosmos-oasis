...
export default function Blog() {
  ...
  return (
    <section className="space-y-6">
      <h1 className="font-['Playfair Display'] text-3xl font-extrabold tracking-tight">Blog</h1>
      <div className="grid gap-6 md:grid-cols-3">
        {posts.map((p) => (
          <Link key={p.id} to={`/blog/${p.slug}`} className="group overflow-hidden rounded-xl border">
            {p.coverImage && (
        ...
        {posts.length === 0 && (
          <div className="rounded-xl border p-6 text-center text-sm text-muted-foreground">No posts published yet.</div>
        )}
      </div>
    </section>
  );
