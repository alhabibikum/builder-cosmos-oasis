  ...
  return (
    <div className="grid gap-8 md:grid-cols-3">
        ...
        {detailed.map((i) => (
          <div key={i.productId} className="flex items-center gap-4 rounded-xl border p-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-muted-foreground">{i.size ? `Size ${i.size} • ` : ""}${i.qty} × ${i.product.price.toFixed(2)}</div>
            ...
            <div className="w-20 text-right font-semibold">${(i.product.price * i.qty).toFixed(2)}</div>
          </div>
        ))}
      </section>
      <aside className="rounded-xl border p-4">
        ...
        <div className="flex justify-between text-sm text-muted-foreground"><span>Shipping</span><span>{total >= 150 ? "Free" : "$15.00"}</span></div>
        <div className="mt-2 flex justify-between border-t pt-2 font-semibold"><span>Total</span><span>${(total + (total >= 150 ? 0 : 15)).toFixed(2)}</span></div>
